...
func (self *Pokemon) calculateSize() {
    self.Size.Numero = int32(unsafe.Sizeof(self.Numero))
    self.Size.Nome = MAX_NAME_LEN
    self.Size.NomeJap = int32(len(self.NomeJap) / 3 * 4)
    self.Size.Geracao = int32(unsafe.Sizeof(self.Geracao))

    date_size, err := self.Lancamento.MarshalBinary()
    if err != nil {
        panic("Opora")
    }

    self.Size.Lancamento = int32(len(date_size))
    self.Size.Especie = int32(len(self.Especie))
    self.Size.Lendario = int32(unsafe.Sizeof(self.Lendario))
    self.Size.Mitico = int32(unsafe.Sizeof(self.Mitico))
    self.Size.Tipo = int32(len(self.Tipo[0]) + 1)
    if len(self.Tipo) > 1 {
        self.Size.Tipo += int32(len(self.Tipo[1]))
    }
    self.Size.Atk = int32(unsafe.Sizeof(self.Atk))
    self.Size.Def = int32(unsafe.Sizeof(self.Def))
    self.Size.Hp = int32(unsafe.Sizeof(self.Hp))
    self.Size.Altura = int32(unsafe.Sizeof(self.Altura))
    self.Size.Peso = int32(unsafe.Sizeof(self.Peso))

    self.Size.Total = self.Size.Numero + 4 +
        self.Size.Nome + 4 +
        self.Size.NomeJap + 4 +
        self.Size.Geracao + 4 +
        self.Size.Lancamento + 4 +
        self.Size.Especie + 4 +
        self.Size.Lendario + 4 +
        self.Size.Mitico + 4 +
        self.Size.Tipo + 4 +
        self.Size.Atk + 4 +
        self.Size.Def + 4 +
