...
func Create(pokemon models.Pokemon) (int, error) {
	...
	hashing.HashCreate(int64(pokemon.Numero), address, binManager.BIN_PATH)
...
func Read(id int) (models.Pokemon, error) {
	pos, err := hashing.HashRead(int64(id), binManager.BIN_PATH)
...
func ReadAll(page int) (pokemon []models.Pokemon, err error) {
	...
	for id, i, total := atual+1, 0, 0; total < 60 && id <= ultimoID && i < numRegistros; i++ {
		pokeAddress, err = hashing.HashRead(int64(id), binManager.BIN_PATH)
...
func Update(pokemon models.Pokemon) (err error) {
	...
	pos, err := hashing.HashRead(int64(pokemon.Numero), binManager.BIN_PATH)
	if err != nil {
		return
	}
	...
	err = hashing.HashUpdate(int64(pokemon.Numero), newAddress, binManager.BIN_PATH)
...
func Delete(id int) (pokemon models.Pokemon, err error) {
	...
	hashing.HashDelete(int64(pokemon.Numero), binManager.BIN_PATH)
